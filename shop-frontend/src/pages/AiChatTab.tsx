  ...
  const switchMode = (m: ChatMode) => {
    ...
    setMessages([
      { role: "ai", text: m === "faq"
        ...
        : "你好！我是智居购物助手，可以直接帮你搜商品、加购物车、下单。请先在下方粘贴你的 JWT Token。" }
    ]);
    setHistory([]);
  };
  ...
  const send = async (text: string) => {
    ...
    try {
      if (mode === "faq") {
        ...
        const r = await api.agentChat(newHistory, userToken);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)]">
      <div className="flex items-center gap-3 mb-3">
        ...
        {mode === "agent" && (
          <input
            className="flex-1 px-3 py-1 border rounded-full text-xs outline-none"
